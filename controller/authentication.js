...
import {
  CustomerOtherInformationValidator,
  MobileValidator,
...
router.post('/get_message_info', async (req, res) => {
  try {
    ...
    res.status(200).send(OK('Notification Send', null, req))
  } catch (e) {
    console.log(e)
...
router.post('/check_mobile', checkModule, MobileValidator, async (req, res) => {
  try {
    ...
    MerchentProfile.findOne({ where: { MSISDN: mobile } })
      .then(async (data) => {
        if (data) {
          const OTP = genRandomInRange(100000, 999999)

          const authtrack = await MerchentUserAuthTrack.findOne({
            where: { MSISDN: mobile },
          })

          if (authtrack && authtrack.password != null) {
            await MerchentUserAuthTrack.update(
              {
                fullname: data.Merchant_Name,
                otp: OTP,
                otp_created_at: currenttimestamp(),
              },
              { where: { MSISDN: mobile } }
            )

            SmsApi(mobile, `${OTP} OTP is user verification code`, 'Y', false)
            ...
            await MerchentUserAuthTrack.create({
              MSISDN: data.MSISDN,
              fullname: data.Merchant_Name,
              otp: OTP,
              otp_created_at: currenttimestamp(),
          ...
          return res
            .status(400)
            .send(BAD_REQUEST(req.i18n.__('usernotregistered'), null, req))
        }
      })
      .catch((e) => {
        console.log(e)
...
router.post('/check_otp', checkModule, MobileValidator, async (req, res) => {
  try {
    ...
    MerchentUserAuthTrack.findOne({ where: { MSISDN: mobile } })
      .then(async (data) => {
        if (data) {
          var now = new Date()
          ...
          if (otp == data.otp) {
            if (
              SecondDifferenceBetweenToDate(data.otp_created_at, utc) <=
              otp_valid_time
            ) {
              await MerchentUserAuthTrack.update(
                { otp_used: 1 },
              ...
              return res
                .status(400)
                .send(BAD_REQUEST(req.i18n.__('otpexpired'), null, req))
            }
          } else {
            return res
              .status(400)
              .send(BAD_REQUEST(req.i18n.__('otpdoesnotmatch'), null, req))
          }
        } else {
          return res
            .status(400)
...
const somePartOfLogin = async (user, req, res, use_temp_password = false) => {
  let { MSISDN, parent_id, fullname, ismanager } = user,
    login_datetime = new Date(),
    logo = null
  let is_merchent = parent_id ? false : true
  let common_id = parent_id || MSISDN,
    isBank = false,
    linked_agent_MSISDN = null

  const merchentinfo = await MerchentProfile.findOne({
    where: { MSISDN: common_id },
  })

  const merchentagentmap = await MerchentAgentProfileMap.findOne({
    where: { Merchent_MSISDN: common_id },
  })
  ...
  if (merchentinfo) {
    logo = merchentinfo.Logo_Image
      ? await getImageFullPath(merchentinfo.Logo_Image)
      : null
  }
...
router.post('/set_password', checkModule, MobileValidator, async (req, res) => {
  try {
    ...
    MerchentUserAuthTrack.update(
      ...
      .then(async (data) => {
        const info = await MerchentUserAuthTrack.findOne({
          where: { MSISDN: mobile },
          logging: console.log,
        })
    ...
    if (info && info.password) {
      ...
      if (is_match) {
        if (info.status == 1) {
          ...
          return res
            .status(400)
            .send(BAD_REQUEST(req.i18n.__('accountpendingmessage'), null, req))
        } else {
          return res
            .status(400)
            .send(BAD_REQUEST(req.i18n.__('accountlockedmessage'), null, req))
        }
      } else {
        return res
          .status(400)
          .send(BAD_REQUEST(req.i18n.__('unauthorized'), null, req))
      }
    } else {
      return res
        .status(400)
